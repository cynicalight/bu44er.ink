---
title: '如何将keil项目移植到pio并编译烧录-VSCode版'
summary: 'keil项目移植到platformIO并编译烧录 with stm32 on macOS via vscode'
date: 2025-05-09
tags:
  - embedded
draft: false
images: ['/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250509131014.webp']
authors: ['default']
---
...
![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250501023604.webp)

### 移植keil项目文件

左边是新建的pio项目，右边是原来的keil项目

![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250501023702.webp)

`Src Inc` 放到对应的 `src include` 中即可

注意，`Drivers` 中的内容，pio会在编译的时候自动放在 `.pio/build`，如下

![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250501023946.webp)

按照上面的方式，默认ini配置下，直接编译就没有错误，并不需要在ini中更改设置。

编译和烧录都在VSCode左下角的底栏：

![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250509125550.webp)

### 烧录

首先需要修改ini文件，配置对应的烧录协议，实验课用的是stlink的仿真器，一般可能是serial，按自己情况来：

![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250509125744.webp)
...
![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250509130108.webp)

可以看到4444端口在监听telnet，我们用telnet连接4444：

![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250509130201.webp)
...
![](/static/images/blog/2024/keil-to-pio/Pasted%20image%2020250509131014.webp)
